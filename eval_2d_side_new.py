...
class Eval2DSide:
    ...
    def eval_2d_side_recall_precision(self):
        ...
        for i in range(len(self.lable_jsons_list)-1):
            ...
            if perce_type_list==[]:
                ...
                for j in range(len(lable_type_list)):
                    lable_center_x = lable_boxs_list[j]["x"] + lable_boxs_list[j]["w"]/2
                    lable_center_y = lable_boxs_list[j]["y"] + lable_boxs_list[j]["h"]/2
                    is_in_attentionArea = utils.judge_is_in_attentionArea(lable_center_x,lable_center_y, attention_area)
                    ...
                    for k in range(len(perce_type_list)):
                        iou_result[k] = utils.bb_intersection_over_union_side(lable_boxs_list[j],perce_boxs_list[k])
                ...
                for t in range(len(perce_type_list)):
                    perce_center_x = (perce_boxs_list[t]["x"] + perce_boxs_list[t]["w"]/2) * self.proportion
                    perce_center_y = (perce_boxs_list[t]["y"] + perce_boxs_list[t]["h"]/2) * self.proportion + self.topcut
                    is_in_attentionArea = utils.judge_is_in_attentionArea(perce_center_x, perce_center_y, attention_area)
                    if (is_in_attentionArea==False) or (perce_type_list[t] not in self.obstacle_type):
                                continue
                    iou_result={}
                    for r in range(len(lable_type_list)):
                        iou_result[r] = utils.bb_intersection_over_union_side(lable_boxs_list[r],perce_boxs_list[t])
                    iou_max_item = max(iou_result.items(), key=lambda x: x[1])
                    iou_max_value = iou_max_item[1]
                    iou_max_id = iou_max_item[0]
                    if iou_max_value >= self.iou:
    ...
    def get_oripic_depend_errorjson(self):
        ...
        for root,_,files in os.walk(self.err_json):
            for file_ in files:
                ...
                if filenum=='vc5_':
                    command = 'cp {}/side_left_rear/{} {}'.format(self.ori_pic_path,filename,dstpic_path)
                    print(command)
                    os.system(command)
                if filenum=='vc6_':
                    command = 'cp {}/side_left_front/{} {}'.format(self.ori_pic_path,filename,dstpic_path)
                    os.system(command)
                if filenum=='vc7_':
                    command = 'cp {}/side_right_front/{} {}'.format(self.ori_pic_path,filename,dstpic_path)
                    os.system(command)
                if filenum=='vc8_':
                    command = 'cp {}/side_right_rear/{} {}'.format(self.ori_pic_path,filename,dstpic_path)
                    os.system(command)
            ...
            for file_ in files:
                ...
                for json_data_ in json_data:
                    ...
                    if perce_type!=None or perce_box!=None:    
                        ...
                        y2 = y2 * self.proportion + self.topcut
                        x3 = x3 * self.proportion
                        y3 = y3 * self.proportion + self.topcut 
        ...
        for root,_,files in os.walk(os.path.join(self.err_pic,'errdet')):
            for file_ in files:
                ...
                for temp in (utils.get_json_data(jsonfile))["task_vehicle"]:
                    ...
                    cv2.putText(imgdata4,(lable_type4[:3]),(x4,y4),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1)
        ...
        for root,_,files in os.walk(os.path.join(self.err_pic,'missdet')):
            for file_ in files:
                ...
                for temp in (utils.get_json_data(jsonfile))["tracks"]:
                    ...
                    y5 = int(temp["uv_bbox2d"]["obstacle_bbox.y"]) * self.proportion + self.topcut
